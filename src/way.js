(function(root, factory) {
    ...
    WAY.prototype.fromJSON = function(data, options, element) {
        ...
        if (options.attr) {
            self.dom(element).setAttr(data, options);
        }
        if (options.data) {
    ...
    WAY.prototype.setAttr = function(data, options, element) {
        ...
        for (var i = 0; i < optionsKeys.length; i++) {
            if (optionsKeys[i].indexOf('attr-') === 0) {
                tmpkey = optionsKeys[i].replace(/^attr-/, '');
                if (typeof validAttributes[tmpkey] === 'undefined') {
                    validAttributes[tmpkey] = {'value': null, 'default': null}
                }
                validAttributes[tmpkey]['value'] = options[optionsKeys[i]];
            } else if (optionsKeys[i].indexOf('default-attr-') === 0) {
                tmpkey = optionsKeys[i].replace(/^default-attr-/, '');
                if (typeof validAttributes[tmpkey] === 'undefined') {
                    validAttributes[tmpkey] = {'value': null, 'default': null}
                }
                validAttributes[tmpkey]['default'] = options[optionsKeys[i]]
            }
        }
        ...
        for (var i = 0; i < optionsKeys.length; i++) {
            ...
            if (value !== null) {
                w.dom(element).attr(optionsKeys[i], value)
            }
        }
    }
    ...
    WAY.prototype.setValue = function(data, options, element) {
        ...
        options.transform.forEach(function(transformName) {
            ...
            data = transform(data, w.dom(element));
        });
        ...
        var setters = {
            ...
            TEXTAREA: function(a) {
                ...
                if (options.template) {
                    var val = w.dom(element).val().replace(templatevar, a || '');
                    w.dom(element).val(val);
                } else {
                    w.dom(element).val(a || '');
                }
            },
            PRE: function(a) {
                if (options.html) {
                    if (options.template) {
                        a = w.dom(element).html().replace(templatevar, a);
                    }
                    w.dom(element).html(a);
                } else {
                    if (options.template) {
                        a = w.dom(element).text().replace(templatevar, a);
                    }
            ...
            IMG: function(a) {
                ...
                if (options.template) {
                    a = w.dom(element).attr('src').replace(templatevar, a);
                }
    ...
    WAY.prototype.registerRepeats = function() {
        ...
        for (var i in elements) {
            ...
            var options = self.dom(element).getOptions();
            var scope = self.dom(element).scope();
            var repeat_selector = scope ? scope + '.' + options.repeat : options.repeat;

            self._repeats[repeat_selector] = self._repeats[repeat_selector] || [];

            var wrapperAttr =
                tagPrefix + '-repeat-wrapper="' + self._repeatsCount + '"',
                parent = w.dom(element).parent('[' + wrapperAttr + ']');
            if (!parent.length) {
                self._repeats[repeat_selector].push({
                    ...
                    element: w
                        ...
                        .get(0),
                    selector: repeat_selector,
                    filter: options.filter
                });

                var wrappertag = options.wrappertag || "div";
                // use parent as wrapper
                if (wrappertag === 'parent') {
                    var wrapper = w.dom(element).parent();
                ...
                self.updateRepeats(repeat_selector);
    ...
    WAY.prototype.updateRepeats = function(selector) {
        ...
        repeats.forEach(function(repeat) {
            ...
            var data = self.get(repeat.selector);
            var items = [];
    ...
    _json.set = function(json, selector, value) {
        ...
        return (value === undefined || value === null || (typeof value === 'number' && isNaN(value)))
            ? _json.remove(json, selector) : deepJSON(json, selector, value);
